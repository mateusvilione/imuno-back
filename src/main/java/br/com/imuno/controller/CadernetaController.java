...
/*
	@Override
	@PutMapping("/{id}")
	public ResponseEntity<?> atualizar(@RequestBody Caderneta caderneta, @PathVariable Long id) {
		Caderneta cadernetaAtual = _service.buscar(id).orElse(null);
		if (cadernetaAtual != null) {
			BeanUtils.copyProperties(caderneta, cadernetaAtual, "id");
			_service.atualizar(cadernetaAtual);
			return ResponseEntity.ok(cadernetaAtual);
		}
		return ResponseEntity.notFound().build();
	}
	...
	@Override
	@DeleteMapping("/{id}")
	public ResponseEntity<Caderneta> excluir(@PathVariable Long id) {
		try {
			_service.excluir(id);
